...
namespace Signals.Core.Web.Http
{
    ...
    public class SessionProvider : ISessionProvider
    {
        ...
        public SessionProvider(System.Web.HttpContext context)
        {
            _context = context;
        }
        ...
        public void Set(string name, object value)
        {
            if (!_context.Session.IsNull())
                _context.Session[name] = value.SerializeJson();
        }
        ...
        {
            var sessionValue = _context.Session?[name];
            if (sessionValue.IsNull()) return (T)null;
            return sessionValue.ToString().Deserialize<T>(SerializationFormat.Json);
        }
